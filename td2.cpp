...
ostream& operator<< (ostream& os, const Film& film)
{
	...
	os << "  Réalisateur: " << film.realisateur_ << "  Année :" << film.anneeSortie << endl;
...
Livre lireLivre(istream& fichier) {
	Livre livre;
	fichier >> quoted(livre.titre_) >> livre.annee_ >> quoted(livre.auteur_) >> livre.recette_ >> livre.nombrePages_;
	cout << livre.titre_<< "  ";
	cout << livre.annee_ << "   ";
	cout << livre.auteur_ << "   ";
	cout << livre.recette_ << "    ";
	cout << livre.nombrePages_ << "   ";
	cout << "         ";
...
int main()
{
	...
	cout << "\nFilm de 955M$:\n" << *film955;
	assert(film955->titre_ == "Le Hobbit : La Bataille des Cinq Armées");
	...
	*listeTextes2[1] = "Allo!";
	...
	for (int i = 0; i < listeFilms.size(); i++) {
		vecteur.push_back(*listeFilms[i]);
	}

	while (!fichierLivres.eof()) {
